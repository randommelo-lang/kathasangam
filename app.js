(function () {
  ...
  function renderStudio() {
    ...
    view.appendChild(el("div", "layout-two", [
      ...
      el("aside", null, [
        el("section", "panel", [el("h2", null, "Create story"), storyForm()]),
        el("section", "panel", [el("h2", null, "Analytics"),
          el("div", "analytics-grid", [metric("Reads", formatNumber(active.views)), metric("Likes", formatNumber(active.likes)), metric("Tips", "$" + formatNumber(active.earnings))]),
          el("div", "bar-chart", [42, 64, 36, 78, 88, 52, 96].map(function (h) { var b = el("span"); b.style.height = h + "%"; return b; }))])
      ])
    ]));
  }
  ...
  function storyForm() {
    return form("storyForm", [
      ...
      submitButton("Create", "btn primary")
    ]);
  }
  ...
  function handleViewSubmit(e) {
    ...
    if (e.target.dataset.form === "storyForm") {
      ...
      apiPost("/stories", { title: fd.get("title"), type: fd.get("type"), genre: fd.get("genre"), description: fd.get("description") }).then(function (resp) {
        ...
        return api("/stories");
      }).then(function (s) { state.stories = s; hydrateGenres(); notify("Story created."); render(); });
    }
